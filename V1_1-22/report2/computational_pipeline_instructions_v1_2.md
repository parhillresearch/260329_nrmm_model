...
**Version:** 1.3 (Aligns with Research Plan v1.17)
...
3. **Enforcement Rate ($\bar{e}$) — v1.3 Specification:** $\bar{e}$ = records where `Initial.Machinery.Compliance == "non-compliant"` / total records (cold-engaged AND warm-engaged combined), per group × phase. Use the MC-field directly; do not re-derive from Stage thresholds. Cold-engaged non-compliant records are included because cold operators are assumed ignorant of policy or avoiding surveillance due to non-compliance — not a clean market counterfactual. Do **not** divide by phase duration. For pooled cells (A1/A2 variable speed; CS Phase A), weight by fleet size before averaging.
