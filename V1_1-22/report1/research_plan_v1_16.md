...
**Version**: 1.17 (Enforcement & Compliance Track Separation) | **Date**: 31 March 2026 | **First estimates required**: 30 March 2026, 17:00
...
2. **Enforcement Rate (**$\bar{e}$**):** Proportion of ALL audited records (cold-engaged and warm-engaged combined) where `Initial.Machinery.Compliance == "non-compliant"`, per group × phase. This is an average annual proportion (not a cumulative count) and must **not** be divided by phase duration. The MC-field definition is authoritative: the earlier Stage-threshold definition (warm-only, `Initial.Stage < threshold`) was found to conflate the enforcement and policy tracks and is superseded. Cold-engaged machines are included in the denominator because cold-engaged non-compliant records represent enforcement exposure (ignorant or surveillance-avoiding operators), not a clean market counterfactual. CS Phase B ($\bar{e}$ high) is expected given the abrupt Stage V mandate.
...
- **M3: Model estimated** (Phase 4 complete — see `260330_step4_output.md`; revised in v1.17, script `260331_step4_revised.R`). Key objects: `lambda_cf_streamA`, `lambda_policy_streamA`, `e_bar_rows`, `lambda_proactive_streamA`, `kw_by_group_stage`. **v1.17 revisions:** (1) ē now computed from MC-field (`Initial.Machinery.Compliance == "non-compliant"`) across all records (cold + warm); (2) λ_Policy estimated from compliant-warm subset only; (3) λ_Proactive = λ_Policy − λ_CF (ē not subtracted). Prior findings from all-warm λ_Total are superseded. Granular Stream B λ_CF (CS: 0.358, CAZ+: 0.363, RoL: 0.219) remain authoritative.
