# 260415_step3_parameter_estimation_v4.R
# Step 3: Parameter Estimation — Trial 4
#
# Changes vs Trial 3 (sprint_log.md):
#   NEW 3.4  Enforcement success rate from Route 4/5 outcomes reported descriptively
#            by group × time period (A1, A2, B1, B2, B3, Phase C if P24 present).
#            Raw counts and per-audit success rate only; not a model parameter;
#            not added to step3_params.
#
# Sub-tasks 3.1–3.3 are identical to Trial 3.
#
# Route definitions (schema.md Table 3):
#   Routes 4+5 = init_mach_emissions_compliant == FALSE (E code in initial reasons)
#   Route 4    = Routes 4+5 AND enforcement_upgrade == TRUE (final_stage > initial_stage)
#                Conservative proxy: machine removals without replacement stage logged
#                are counted as Route 5 (lower bound on success rate).
#   Route 5    = Routes 4+5 AND enforcement_upgrade == FALSE
...
# ── Script constants ───────────────────────────────────────────────────────────
SCRIPT_STEM <- "260415_step3_parameter_estimation_v4"
...
# =============================================================================
# Route 4  = init_mach_emissions_compliant == FALSE AND enforcement_upgrade == TRUE
#            (final_stage > initial_stage; machine replaced with higher-stage unit)
# Route 5  = init_mach_emissions_compliant == FALSE AND enforcement_upgrade == FALSE
#            (no stage change; includes machine removals without replacement stage
#             recorded — conservative lower bound on enforcement success)
# Routes 4+5 together represent all emissions-non-compliant audit records.
#
# Broken down by group × time period: A1, A2, B1, B2, B3, C.
# Phase C included if P24 records exist for that group (Constant_Speed,
# Variable_Speed only; CAZ_Plus and Rest_of_London merged out from 1.1.2025).
...
enf_counts <- routes45 |>
  ...
  summarise(
    n_R4    = sum(enforcement_upgrade == TRUE,  na.rm = TRUE),
    n_R5    = sum(enforcement_upgrade == FALSE, na.rm = TRUE),
    n_total = n(),
    .groups = "drop"
  )
...
md(
  "",
  paste0("n R4 = machines driven compliant via emissions upgrade (Route 4; proxy: final_stage > initial_stage);",
         " n R5 = emissions non-compliant at final audit (Route 5; includes removals without replacement stage);",
         " Rate% = n R4 / n tot, the per-audit enforcement success rate; \u26a0 marks cells where n tot < ",
         ENF_SPARSE_N, " (too sparse to interpret reliably)."),
  paste0("Variable_Speed appears only in Phase C (P24 zone post-2025 merger); CAZ_Plus and Rest_of_London",
         ...
         " Route 4 proxy is conservative: machine removals without a replacement stage logged are counted as Route 5."),
  ""
)
...
md(
  ...
  paste0("| **Route 4** | Audit outcome: initial emissions non-compliant; enforcement actioned;",
         " machine replaced or upgraded to a higher emissions stage (final_stage > initial_stage).",
         " Proxied here by the enforcement_upgrade flag. |"),
  paste0("| **Route 5** | Audit outcome: initial emissions non-compliant; enforcement requested",
         " but not actioned; machine remains at original stage. Also captures removals where",
         " no replacement stage is recorded (conservative lower bound). |"),
  paste0("| **Enforcement success rate** | n(Route 4) / n(Routes 4+5): the fraction of",
         " emissions-non-compliant audit records where enforcement resulted in an emissions",
         " stage upgrade. Descriptive policy metric only; not used in transition matrices. |"),
...
cat("\n=== Step 3 Parameter Estimation v4: Objects saved ===\n")
