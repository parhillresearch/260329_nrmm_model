...
# nrmm_dashboard_v2 — unified dashboard over nrmm_model_v3.rds.
#
...
OUTPUT_FILE <- "nrmm_dashboard_v2.html"
...
graph <- visNetwork(init_nodes, init_edges, width = "100%", height = "100%") %>%
  ...
  visExport(type = "pdf", name = "nrmm_dashboard_v2", label = "Export as PDF")
...
js_code <- "function(el, x, data) {
  ...
  function renderEF() {
    ...
    function slider(id, label, s, val) {
      ...
      return '<div style=\"padding:2px 0;\">' + label +
        ...
        s.load_factor + ', EF_t ' + s.ef_type.toFixed(2) + '</span></div>';
    }
    ...
    html += '<table style=\"border-collapse:collapse;font:12px Arial;\">' +
      '<tr>' + ['Machine type','n','mean kW','EF_t','h/day central','h/day low','h/day high','load factor'].map(function(h) {
  ...
  function renderStagePop() {
    ...
    pane.innerHTML =
      ...
      'projected 2025-2030 faded at the slider p-bar</span></div>' +
      legend + stagePanel(group, 'warm_AT') + stagePanel(group, 'cold_CF') +
      '<div style=\"padding:8px 0;\">p-bar scenario multiplier x <b><span id=\"pk2Val\">' +
      pbarK.toFixed(2) + '</span></b> <input type=\"range\" id=\"pk2Slider\" min=\"0\" max=\"2\" ' +
      'step=\"0.05\" value=\"' + pbarK + '\" style=\"width:300px;vertical-align:middle;\"> ' +
      '<span style=\"color:#888;\">fitted p-bar: ' +
      pdw.map(function(r){ return r.arm + ' ' + r.p_fit.toFixed(3); }).join(', ') + '</span></div>' +
  ...
  function redraw() {
    ...
    if (view === 'stagepop') {
      document.getElementById('summary').innerHTML =
        'Observed and projected stage shares by year; absolute populations need the registration multiplier.';
    }
    if (view === 'tree') {
...
htmlwidgets::saveWidget(graph, OUTPUT_FILE, selfcontained = TRUE,
                        title = "NRMM unified dashboard v2")
