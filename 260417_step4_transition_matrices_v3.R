# 260417_step4_transition_matrices_v3.R
...
#   - All Phase C matrices are 6×6 (Stage I–V plus ZE ceiling). CS max_stage=3 was
#     estimation-only; Phase C uses FORECAST_MAX_STAGE=6 for all groups (P24 data
#     shows Stage V CS machines in Phase C).
#   - p_cf = lambda_CF / avg_stage_jump. avg_stage_jump is always computed using
#     FORECAST_MAX_STAGE=6 as the destination ceiling (not the estimation-window
#     ceiling). This ensures p and lambda are denominated in the same stage space
#     as the forecast matrix.
...
#     Matrix display: one kable per group (stages as axes).
#
# v3 fix vs v2: compute_avg_jump now uses FORECAST_MAX_STAGE (6) as both
#   stage cap and ceiling for all groups. v2 used EST_MAX_STAGE=3 for CS,
#   which denominated p_cf in the 3-stage estimation space and then applied it
#   in the 6-stage forecast matrix, producing lambda(P_Natural)=0.342 vs
#   locked lambda_CF=0.051 (6.7× discrepancy). Fixed avg_jump ≈ 3.22 → p_cf ≈ 0.016.
#
# Inputs:
...
# Outputs (outputs/):
#   260417_step4_transition_matrices_v3.md
...
SCRIPT_STEM <- "260417_step4_transition_matrices_v3"
...
B3_START <- 2023.5534   # start of Phase B3 segment
...
#' Build P_Natural: non-top stages transition to FORECAST_MAX_STAGE with prob p_cf per year.
#' Arguments:
...
#' lambda_implied = Σᵢ πᵢ × (E[stage after 1 year | start at i] − i)
#' Arguments:
...
# step3_params loaded for reference only; lambda values are read from locked
# constants above (decisions.md / schema.md Table 8)
...
# avg_stage_jump is computed using FORECAST_MAX_STAGE=6 as both the stage cap
# and the destination ceiling for all groups. This denominates p in the same
# 6-stage space as the forecast matrices.
# CS estimation window: cold records 2016–2023 (2024 excluded per decisions.md).
...
compute_avg_jump <- function(dat, grp, phase_bounds) {
  ...
  sub <- sub |>
    mutate(stage_capped = pmin(initial_stage, FORECAST_MAX_STAGE)) |>
  ...
  avg_jump <- FORECAST_MAX_STAGE - wmean
  list(wmean = wmean, n = nrow(sub), avg_jump = avg_jump)
}

jump_cs  <- compute_avg_jump(audits, "Constant_Speed",  PHASE_BOUNDS)
jump_caz <- compute_avg_jump(audits, "CAZ_Plus",        PHASE_BOUNDS)
jump_rol <- compute_avg_jump(audits, "Rest_of_London",  PHASE_BOUNDS)
...
# Implied lambda is distribution-dependent; residual deviations from locked
# values arise because the Phase C stage distribution differs from the
# estimation-window mean stage. For CS the avg_jump fix (6-space vs 3-space)
# should close the large v2 gap; any remaining deviation <20% is acceptable.
LAMBDA_DIVERGENCE_THRESHOLD <- 0.20   # warn if |implied - locked| / locked > this
...
val_A <- map_dfr(PHASE_C_GROUPS, function(grp) {
  ...
  lam_nat      <- implied_lambda(m$P_Natural, pi_i, FORECAST_MAX_STAGE)
  lam_tot      <- implied_lambda(m$P_Total,   pi_i, FORECAST_MAX_STAGE)

  # Warn on large divergence between implied and locked lambda_CF
  if (lam_cf_lock > 0) {
    rel_dev <- abs(lam_nat - lam_cf_lock) / lam_cf_lock
    if (rel_dev > LAMBDA_DIVERGENCE_THRESHOLD) {
      warning(sprintf(
        "%s: lambda(P_Natural)=%.4f deviates %.0f%% from locked lambda_CF=%.4f (threshold %d%%)",
        grp, lam_nat, rel_dev * 100, lam_cf_lock, LAMBDA_DIVERGENCE_THRESHOLD * 100
      ))
    }
  }
  ...
  tibble(
    ...
    `lambda(P_Natural)`                            = lam_nat,
    `lambda_Pro (locked)`                          = lam_pro_lock,
    `lambda(P_Total)`                              = lam_tot,
...
val_B_plot_dat <- val_B_rows |>
  pivot_longer(all_of(unname(STAGE_LABELS)), names_to = "stage", values_to = "prop") |>
  mutate(
...
cat("\n=== Step 4 Transition Matrices (v3): Objects saved ===\n\n")

cat("avg_stage_jump (forecast space, ceiling = Stage V = 6):\n")
for (grp in GROUP_ORDER) {
  jmp <- step4_matrices$avg_jump[grp]
  cat(sprintf("  %-18s avg_jump = %.4f\n", grp, jmp))
}
...
cat("\nLambda validation (implied vs locked):\n")
for (i in seq_len(nrow(val_A))) {
  cat(sprintf("  %-18s lambda(P_Natural)=%.4f  locked=%.4f\n",
              val_A$Group[i],
              val_A$`lambda(P_Natural)`[i],
              val_A$`lambda_CF (locked)`[i]))
}
