...
# nrmm_model_v1 — unified NRMM model: one classified data spine, three layers.
#
...
output_md <- "outputs/nrmm_model_v1.md"
...
# --- Layer 2: EF strata per Machine Group x phase ---
...
# observed yearly c-bar per Machine Group x arm (exact-date thresholds)
...
md <- c(
  "# nrmm_model_v1 — unified model: outcomes, EF, dynamics (260717)",
  "",
...
nrmm_model_v1 <- list(
  outcomes = outcomes, ef_results = ef_results, strata = strata,
  usage_index = USAGE_INDEX, cell_summary = cell_summary, pairs = pairs,
  pbar = pbar, projections = projections, observed_trend = observed_trend,
  proj_init = proj_init, stage_ef = stage_ef, nox_limit = NOX_LIMIT
)
saveRDS(nrmm_model_v1, "intermediate_data/nrmm_model_v1.rds")

manifest_path <- "intermediate_data/manifest.md"
manifest_row <- paste0(
  "| Model | nrmm_model_v1 | nrmm_model_v1.rds | list | ",
  nrow(outcomes), " outcome cells; ", nrow(pbar), " p-bar fits; ",
  nrow(projections), " projection rows | ",
  "Unified NRMM model: outcome taxonomy aggregates, type-stratified EF, ",
  "two-era constrained Markov p-bar and 2025-2030 projections. |"
)
...
cat("Completion summary: nrmm_model_v1 (list), ", length(nrmm_model_v1),
