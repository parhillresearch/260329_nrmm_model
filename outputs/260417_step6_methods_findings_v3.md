...
_Version 3 | 17 April 2026_  
_Script: `outputs/260417_step6_methods_findings_v3.md` (Step 6, no R script)_  
...
_π₀: Phase C initialisation distribution (Stage proportions of cold-engaged P24 records at the start of the forecast period). λ: fleet improvement rate in stage integers per year (weighted least squares slope). ē: non-compliance exposure rate (% of machines below the compliance threshold). pp: percentage points. Symbols and abbreviations defined in the Glossary._
...
| Yes | **Variable\_Speed** | Merged CAZ+ and Rest of London from 1.1.2025. P24 zone only. | `Engine.Type.clean == "Variable"` AND `Zone == "P24"` |
...
![Variable\_Speed Phase C stage distributions by engagement type](260414_step2_exploratory_v7_1_stage_variable_speed.png)
...
**Execution:** Weighted least squares (WLS) fitted via `lm(mean_stage ~ mid, weights = n)` with standard error (SE) extracted from `vcov()`. Constant\_Speed used annual midpoints 2016–2023 (2024 excluded). Variable-speed groups (CAZ+, RoL) used A1/A2 pooled as a single point plus Phase B 3-segment midpoints. Stages were capped at group-specific `MAX_STAGE` (CS: 3; CAZ+/RoL: 6) before averaging.
...
**Intended task:** Estimate e-bar (ē) from all records where Initial Stage < compliance threshold for that group and phase (schema.md Table 2); do not divide by phase duration.

**Execution:** For each group × phase cell, records with `initial_stage < COMP_THRESH[[phase]][[group]]` were counted and expressed as a percentage of all records in that cell. The compliance threshold for each group and policy era determines which machines are counted as non-compliant in the ē calculation:
...
`COMP_THRESH` encoded these thresholds as integers (e.g. Constant\_Speed Phase A1/A2: Stage IIIA = integer 3; Phase B: Stage V = integer 6).
...
_End of report. Version 3 generated 17 April 2026._
