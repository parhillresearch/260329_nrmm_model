# 260415_step3_parameter_estimation_v1.R
# NRMM LEZ Trend Analysis — Step 3: Parameter Estimation (Trial 1)
#
...
#   - CAZ+ max_stage = 5; Phase B primary via 3-segment temporal split
...
#
# Outputs (outputs/):
#   260415_step3_parameter_estimation_v1.md
...
# A1 and A2 share the same policy era requirements
COMP_THRESH <- list(
...
#' @param max_stage  integer: number of active stages
...
save_table_md(e_bar_rows |> select(-threshold_int),
              caption = paste0("e-bar — proportion of all records below compliance threshold",
...
message("Warm self-compliant records by group × phase:")
warm_self |>
  count(group, phase) |>
  pivot_wider(names_from = phase, values_from = n, values_fill = 0L) |>
  print()
...
# ── 3b. CAZ_Plus — Phase B 3-segment, max_stage=5 ────────────────────────────
...
e_bar_B <- e_bar_rows |> filter(phase == "B") |> select(group, e_bar_B = e_bar)
e_bar_A <- e_bar_rows |> filter(phase %in% c("A1", "A2")) |>
  group_by(group) |>
  summarise(e_bar_A = round(mean(e_bar), 4), .groups = "drop")
...
manifest_entries <- tibble(
  ...
  description = c(
    ...
    "e-bar: proportion below compliance threshold, all records, group x phase",
