...
# ════════════════════════════════════════════════════════════════════════════
# SECTION C: ē — Enforcement Rate (v1.17)
# Numerator:   ALL records where Initial.Machinery.Compliance == "non-compliant"
# Denominator: ALL records (cold + warm) per group × phase
# Cold non-compliant included: cold operators assumed ignorant/avoidant,
#   not a clean market counterfactual.
# ════════════════════════════════════════════════════════════════════════════
cat("\n══ ENFORCEMENT RATE (ē) — v1.17: MC-field, all records ══\n")
cat("   [Initial.Machinery.Compliance == 'non-compliant' / n_all per group × phase]\n\n")
...
cat("\nComparison — overall non-compliance rate by fleet type:\n")
cat(sprintf("  All records:  %.1f%%\n", 100 * mean(!df$mc_compliant)))
cat(sprintf("  Warm only:    %.1f%%\n", 100 * mean(!df_warm$mc_compliant)))
cat(sprintf("  Cold only:    %.1f%%\n", 100 * mean(!df_cold$mc_compliant)))
