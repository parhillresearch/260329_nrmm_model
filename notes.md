...
Current model: `nrmm_model_v1.R`, one classified data spine with three layers (outcome taxonomy, type-stratified EF, two-era constrained Markov dynamics), saved as `intermediate_data/nrmm_model_v1.rds`, results in `outputs/nrmm_model_v1.md`. Full documentation in the next section. It supersedes the pre-audit lambda model (parameters archived below), the phase-stratified estimation design, and the separate derivations in `tree_dashboard_v3-v5.R` and `260717_ef_fleet_v1-v2.R`, which remain as development history.
