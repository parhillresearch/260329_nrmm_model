...
- **260728 (later) - query answered: "Electric" = zero emission.** Audit team confirmed; implemented in `nrmm_model_v6.R` / `nrmm_dashboard_v6.R`, mapping "Electric" to stage 7 alongside "ZE". Impact is small today but directionally right and will grow: 16 records gain a usable stage (1 in 2022, 2 in 2023, 13 in 2024); CAZ+ phase B compliance +0.12 pp cold / +0.06 pp warm; fleet EF phase B 2.519 → 2.515, CAZ+ 1.773 → 1.767; variable-speed p-bar 0.2311 → 0.2329 pooled. Modest because 15 of the 31 Electric records fall in 2025 and are already excluded for the unrelated P24 reason, so the fix will matter more once P24 is implemented and as electrification continues. "Electric" no longer appears in `unmapped_values`. **Four items remain blocked**, all with the audit team.
