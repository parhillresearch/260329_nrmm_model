...
> Superseded by: notes.md "State of play" and the current model documentation.
...
5. Generate basic statistics and stacked bar charts by year for: (a) excluded records by exclusion reason; (b) Stage distributions (all records and faceted by group); (c) non-compliance codes — **use `init_mach_emissions_compliant` (pre-derived boolean flag, immune to "None" string collision) as the compliant/non-compliant split; display non-compliant breakdown by NC hierarchy (E > R > Other) as stacked bars; overlay `final_mach_emissions_compliant == TRUE` as a "became compliant" layer to show audit-driven compliance uplift**; (d) group × engagement type; (e) Variable_Speed continuous (vs_member); (f) pre-2016 records characterisation.
...
**Trial 5 key tasks (fixes Trial 4 compliance definition failure):**

- **Sub-task 5c fix:** Replace `classify_nc_top()` string-regex approach with `init_mach_emissions_compliant` boolean flag for all compliant/non-compliant splits. The pre-derived flag uses `!grepl("E", initial_machinery_reasons, fixed = TRUE)` — `fixed = TRUE` is case-sensitive and matches literal 'E', so the word "None" (standard audit entry for no reason code) is correctly treated as compliant. Do not re-derive compliance from raw strings in section 8c.
- **"Became compliant" overlay:** Add `final_mach_emissions_compliant` as a second layer. Plot: stacked bar showing (bottom) `init_mach_emissions_compliant == FALSE & final_mach_emissions_compliant == TRUE` ("became compliant"), (middle) `init_mach_emissions_compliant == FALSE & final_mach_emissions_compliant == FALSE` ("remained non-compliant"), broken down by NC hierarchy (E > R > Other) for the non-compliant portion only; reference line for compliant totals. This directly visualises enforcement-driven compliance uplift.
- **Logic-bug audit of all pipeline assignment tasks:** For every derived boolean or categorical flag in the Step 1 pipeline, verify the derivation logic cannot be fooled by standard data-entry conventions ("None", mixed case, empty strings, NA). Specifically audit: `cold_engaged`, `emissions_compliant` / `admin_compliant` derivations (List 2 code detection), `initial_stage` / `final_stage` encoding (List 1), zone classification, group assignment `case_when`, phase assignment, `no_power_rating` flag, `engine_type_clean`. For each, document: (a) the field values actually present in the data; (b) whether the derivation correctly handles "None", NA, and mixed case; (c) any found bugs and the fix applied.
