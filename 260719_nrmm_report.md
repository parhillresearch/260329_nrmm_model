...
This report explains, in plain English, how we analysed nine years of machine audits from London's Non-Road Mobile Machinery (NRMM) Low Emission Zone, what we found, and how confident we can be. Technical detail sits in boxed notes for specialist readers; the main thread can be read without them. An interactive dashboard accompanies this report (`nrmm_dashboard_v3.html`), including a glossary of terms.

---

## 1. Summary

- **The scheme's biggest effect happens before an inspector ever arrives.** Construction fleets that register with the scheme turn up with cleaner machines than fleets that never register: 89% of registered machines already met the required standard on first sight, against 81% of unregistered ones, and the average registered machine emits roughly 16% less NOx per unit of work on arrival.
...
- **Most "resolved" non-compliant machines simply left the site, and many did not leave London.** At least 43% of removed machines with traceable serial numbers turn up again at other audited sites. Removal often moves pollution rather than reducing it.
...
The 5% was too generous in one way: nearly all of it was machines removed from site, counted as if their emissions had ended. Tracing serial numbers showed at least 43% of traceable removed machines working again at other audited sites later. It was too harsh in another way: it looked only at the enforcement route and completely missed the largest effect, the cleaner machines that registered fleets bring through the gate in the first place. It also mixed paperwork compliance into the emissions story: several hundred records became "compliant" through registration or exemptions with no emissions change at all.

The corrected accounting: 108 audits (0.9%) with a demonstrated emissions reduction; 132 more (1.1%) where the machine plausibly left the zone for good; around 390 removals whose effect is unknowable with current data; and 362 exemptions plus 640 registration-only cases that should never be counted as emissions gains.

> **For technical readers.** Weighting by indicative NOx rather than counting audits: confirmed reductions are worth ~0.2% of audited-fleet NOx (most non-compliant machines are resolved by removal, not upgrade), probable exits add ~2.6% if counted as eliminated, and ~7.4% hangs on the unresolved removals. The enforcement channel is therefore bounded between roughly 0.2% and 10% of fleet NOx depending entirely on removal fate, which argues for treating removal as displacement pending evidence, and for the registration database as the way to settle it.

## 5. The policy's main effect: cleaner fleets arrive

Comparing the two arms at first sight of each machine, registered fleets arrive substantially cleaner. Over the whole period, 89.3% of registered machines were compliant at arrival against 80.8% of unregistered ones, a gap of 8.5 percentage points, and the gap holds within each policy phase (12% to 26% cleaner in emissions-intensity terms, growing over time). Since registered and unregistered fleets turn out to renew their machines at the same rate once on site (section 6), this arrival gap is the clearest measurable footprint of the policy: operators sort their fleets before coming through the gate.

> **For technical readers.** Arrival emissions intensity uses each stage's legal NOx limit at the machine's power band as the practical value (9.2 g/kWh Stage I down to 0.4 at IV/V for 56-560 kW). Warm vs cold mean arrival EF: 2.00 vs 2.38 g/kWh fleet-wide; excluding machines with pre-existing dispensations, the warm fleet arrives 12.1% (phase A1), 17.0% (A2) and 26.0% (B) cleaner per machine. Selection effects cut in the conservative direction: cold machines were found by officers, so the true unregistered fleet is likely dirtier than measured.
...
- **Interactive dashboard:** `nrmm_dashboard_v3.html` (open in any browser; six views including a glossary; all figures in this report can be reproduced there).
- **Model:** `nrmm_model_v3.R` builds everything from the raw audit file in one run; results tables in `outputs/nrmm_model_v3.md`.
